...
| v2.62 | 2026-03-31 09:40 | arch | 巡检: master 8633b69, 532测试通过. 重新rebase fix/swarm-sort-determinism onto master并推送(a4306c1). gh CLI仍无API认证(git push OK但gh pr create失败), PR #301需手动创建或修复gh auth. evomap.ai GEP Protocol Ch16调研: 6种资产类型(Gene/Capsule/EvolutionEvent/Mutation/ValidationReport/MemoryGraphEvent)全部字段已覆盖, schema_version 1.5.0✅, epigenetic_marks✅, multi-language signals_match✅. 黑板网络异常(连接失败). 项目稳定, 无实质新开发任务, 无需创建额外PR |
| v2.60 | 2026-03-31 09:20 | arch | 巡检: master 0436883, 532测试通过, 发现flaky test bug(swarm排序). 修复src/swarm/engine.ts: 增加swarm_id作为secondary tiebreaker解决排序不稳定问题(23b58d4). 分支fix/swarm-sort-determinism已推送, PR待合并. 项目稳定 |
