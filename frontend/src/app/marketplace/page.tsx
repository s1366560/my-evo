...
export default function MarketplacePage() {
  ...
  const getTypeColor = (type: string) => type === 'GENE' ? 'bg-emerald-600 text-black' : 'bg-violet-800 text-white';
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100">
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={() => setTypeFilter("GENE")} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${typeFilter === "GENE" ? "bg-emerald-600 text-black" : "bg-gray-800 text-gray-300 hover:bg-gray-700"}`}>Genes</button>
          ...
          <div className="flex gap-2 items-center">
            <button onClick={() => fetchMarketplaceData()} disabled={isRefreshing} className="p-2 bg-gray-800 hover:bg-gray-700 rounded-lg transition-all disabled:opacity-50" aria-label="Refresh marketplace data">
              <svg aria-hidden="true" className={`w-5 h-5 ${isRefreshing ? "animate-spin" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
            </button>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400">{searchQuery ? "No assets match your search." : "No assets available yet."}</div>
        ) : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {paginatedAssets.map(asset => (
                <div key={asset.id || asset.assetId} className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden hover:border-gray-700 transition-all duration-200 hover:shadow-lg hover:shadow-purple-500/5 hover:-translate-y-0.5">
                  <div className="p-5">
                    <div className="flex items-start justify-between mb-3">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${getTypeColor(asset.type)}`}>{asset.type}</span>
                    ...
                    <div className="flex flex-wrap gap-1 mb-3">
                      ...
                      {asset.tags?.length > 3 && <span className="px-2 py-0.5 bg-gray-800 rounded text-xs text-gray-400">+{asset.tags.length - 3}</span>}
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-400">{new Date(asset.createdAt).toLocaleDateString()}</span>
                      {asset.model && <span className="text-gray-400 text-xs">{asset.model}</span>}
                    </div>
                  </div>
...
function generateMockAssets(): Asset[] {
  return [
    ...
    { id: "5", assetId: "gene_003", type: "GENE", name: "Translation Pro Gene", description: "High-quality multilingual translation gene supporting 40+ languages.", tags: ["translation", "multilingual", "nlp"], gdiScore: 0.81, model: "gpt-4", nodeId: "node_004", createdAt: new Date(Date.now() - 345600000).toISOString(), status: "PUBLISHED" },
    { id: "6", assetId: "capsule_003", type: "CAPSULE", name: "API Design Helper", description: "Capsule that helps design RESTful APIs with best practices.", tags: ["api", "backend", "development"], gdiScore: 0.76, model: "claude-3", nodeId: "node_005", createdAt: new Date(Date.now() - 432000000).toISOString(), status: "PUBLISHED" },
    { id: "7", assetId: "gene_004", type: "GENE", name: "SQL Query Optimizer", description: "Gene that optimizes SQL queries for performance and readability.", tags: ["sql", "database", "performance"], gdiScore: 0.83, model: "gpt-4", nodeId: "node_006", createdAt: new Date(Date.now() - 518400000).toISOString(), status: "PUBLISHED" },
    { id: "8", assetId: "capsule_004", type: "CAPSULE", name: "UI/UX Review Capsule", description: "Automated UI/UX review capsule following design system guidelines.", tags: ["ui", "ux", "design"], gdiScore: 0.79, model: "gpt-4", nodeId: "node_007", createdAt: new Date(Date.now() - 604800000).toISOString(), status: "PUBLISHED" },
  ];
}
