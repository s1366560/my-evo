...
describe('AIService', () => {
  ...
  describe('generateEdges', () => {
    test('returns one edge', async () => {
      const result = await aiService.generateEdges({ sourceNodeId: 'src-1' });
    ...
    test('uses supplied targetNodeId when present', async () => {
      const result = await aiService.generateEdges({ sourceNodeId: 'a', targetNodeId: 'b' });
    ...
    test('uses supplied label and edgeType when present', async () => {
      const result = await aiService.generateEdges({
        ...
        label: 'causes', edgeType: 'causation',
      });
      expect(result.edges[0].label).toBe('causes');
      expect(result.edges[0].edgeType).toBe('causation');
    });

    test('falls back to a generated target when missing', async () => {
      const result = await aiService.generateEdges({ sourceNodeId: 'a' });
  ...
  describe('generateContext', () => {
    test('returns a string for the "explain" task', async () => {
      const result = await aiService.generateContext({ mapId: 'm1', task: 'explain' });
    ...
    test('returns different content for each task type', async () => {
      const e = await aiService.generateContext({ mapId: 'm', task: 'explain' });
      const s = await aiService.generateContext({ mapId: 'm', task: 'summarize' });
      const r = await aiService.generateContext({ mapId: 'm', task: 'refine' });
      const x = await aiService.generateContext({ mapId: 'm', task: 'expand' });
    ...
    test('falls back to explain content for unknown tasks', async () => {
      const e = await aiService.generateContext({ mapId: 'm', task: 'explain' });
      const u = await aiService.generateContext({ mapId: 'm', task: 'nonexistent' as any });
