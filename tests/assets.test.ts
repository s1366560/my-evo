...
function makeCapsule(overrides: Partial<Capsule> = {}): Capsule {
  return {
    ...
    content: '// This is the complete retry logic implementation with exponential backoff and jitter for handling connection timeouts gracefully',
    diff: '--- a/net.py\n+++ b/net.py\n@@ -10,6 +10,8 @@\n+pool = ConnectionPool(max=10)\n+retry_config = { maxRetries: 3, backoff: exponential }',
    strategy: ['Implement retry with exponential backoff and jitter', 'Add connection pool with max connections'],
    confidence: 0.87,
