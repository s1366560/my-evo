...
// ===== PERFORMANCE: Search Results Cache =====
...
export async function search(query: SearchQuery, prismaClient?: PrismaClient): Promise<SearchResult> {
  // ===== PERFORMANCE: Check cache for first page queries =====
  if (query.offset === undefined || query.offset === 0) {
    const cacheKey = cacheKeys.searchResults(JSON.stringify(query));
    const cached = searchCache.get<SearchResult>(cacheKey);
    if (cached) {
      return { ...cached, query_time_ms: 0 };
    }
  }
  ...
  // ===== PERFORMANCE: Cache first page results =====
  if (offset === 0) {
    const cacheKey = cacheKeys.searchResults(JSON.stringify(query));
    searchCache.set(cacheKey, result, CACHE_TTL.SEARCH_RESULTS);
  }
