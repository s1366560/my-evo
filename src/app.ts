...
import { createMonitoringState } from './monitoring/service';
import { createMonitoringMiddleware } from './monitoring/middleware';

// Shared monitoring state — single instance for both middleware and routes
...
export async function buildApp() {
  ...
  await app.register(fastifySwagger, {
    openapi: {
      ...
      tags: [
        ...
        { name: 'Feedback', description: 'User feedback & UX analytics' },
        ...
        { name: 'Webhook', description: 'Webhook event subscriptions' },
        { name: 'Export', description: 'Data export and download' },
      ],
    },
  });
  ...
  // ── Monitoring middleware (request lifecycle metrics) ─────────────────────
  const monitoringMiddleware = createMonitoringMiddleware(monitoringState);
  ...
  // Global error handler — also records metrics via monitoring middleware
  app.setErrorHandler(async (error: Error & { validation?: unknown; statusCode?: number }, request, reply) => {
    const rawUrl = request.url ?? '';
    const path = rawUrl.split('?')[0] ?? rawUrl;
    const excludedPaths = ['/health', '/metrics', '/dashboard/metrics'];
    if (!excludedPaths.includes(path)) {
      const { recordMetric, triggerAlert } = await import('./monitoring/service');
      await recordMetric(monitoringState, 'http.errors', 1, {
        error_type: error.name || 'Error',
        path,
      });
      if ((reply.statusCode ?? 500) >= 500) {
        await triggerAlert(monitoringState, '5xx Server Error', 'critical',
          `5xx error on ${path}: ${error.message}`, 'http.5xx_count', 1);
      }
    }
  ...
  // Register enhanced session management routes
  const { sessionManagementRoutes } = await import('./session/management-routes');
  ...
  const { feedbackRoutes } = await import('./feedback/routes');
  await app.register(feedbackRoutes, { prefix: '/api/v2/feedback' });
  ...
  // Register GEP (Genome Evolution Protocol) routes
  ...
  const { exportRoutes } = await import('./export/routes');
  const { batchRoutes } = await import('./batch/routes');
  const { advancedSearchRoutes } = await import('./advanced-search/routes');
  const { auditRoutes } = await import('./audit/routes');
  const { webhookRoutes } = await import('./webhook/routes');

  await app.register(exportRoutes, { prefix: '/api/v2/export' });
  await app.register(webhookRoutes, { prefix: '/api/v2/webhook' });
  await app.register(batchRoutes, { prefix: '/api/v2/batch' });
  await app.register(advancedSearchRoutes, { prefix: '/api/v2/advanced-search' });
  await app.register(auditRoutes, { prefix: '/api/v2/audit' });
  ...
  // Register enhanced RBAC routes
  const { rbacRoutes } = await import('./security/rbac-routes');
  ...
  // EvoMap - Evolutionary Map routes
  const mapRoutes = (await import('./map/routes')).default;
  await app.register(mapRoutes, { prefix: '/api/v2/map' });

  const { default: workspaceRoutes } = await import('./workspace/routes');
